                    ...
                    <div key={task.id} className="p-4 rounded-xl bg-glass border border-glass-border hover:bg-glass-hover transition-colors">
                      ...
                      {(task.proofUrl || task.proofLink) && (
                        <div className="flex flex-wrap gap-2 mb-3">
                          {task.proofUrl && task.proofUrl.split(',').map((url, idx) => (
                            <a key={idx} href={url} target="_blank" rel="noreferrer" className="text-[10px] bg-blue-500/10 text-blue-400 px-2 py-1 rounded-md flex items-center gap-1 hover:bg-blue-500/20 transition-colors">
                              <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
                              Image {task.proofUrl!.split(',').length > 1 ? idx + 1 : ''}
                            </a>
                          ))}
                          {task.proofLink && (
